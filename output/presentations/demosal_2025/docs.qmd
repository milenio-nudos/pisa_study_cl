...
pacman::p_load(dplyr, haven, psych, purrr, tidyr, sjPlot, ggplot2,
               ...
               sjlabelled, ggrepel,forcats,ggpubr)
...
<style>
  ...
  .circular-image {
    ...
    border-radius: 50%;
    object-fit: cover;
    border: 3px solid #5f5758;
  }
</style>
```

::: image-container
<img src="images/d_miranda.jpg" class="circular-image"/> <img src="images/n_tobar.jpg" class="circular-image"/> <img src="images/jc_castillo.jpg" class="circular-image"/>
:::

::: image-container
<img src="images/i_aguayo.jpg" class="circular-image"/> <img src="images/t_urzua.jpg" class="circular-image"/>
:::

## Punto de inicio

::: {.incremental .highlight-last style="font-size: 110%;"}
-   La digitalidad se puede comprender como un fenómeno multidimensional en tanto interactúa constantemente con las esferas económica, social, política y cultural [@ramirez-castaneda_brecha_2018; @camacho_brecha_2005].

-   Existen brechas digitales en relación al acceso de dispositivos digitales, pero últimamente ha cobrado relevancia la brecha digital en torno a **los conocimientos y habilidades** que son necesarios para desenvolverse adecuadamente en entornos digitales [@sunkel_integracion_2014].
...
::: {.incremental .highlight-last style="font-size: 110%;"}
-   Autoeficacia Digital: "*Juicio introspectivo respecto a el grado de maestría o habilidad percibido para ejecutar acciones que requieran lidiar con situaciones prospectivas en los entornos digitales (hacer arreglos en las acciones para ajustarlas a la búsqueda de un objetivo)*" [@bandura_psychology_2006; @ulfert-blank_assessing_2022a].

-   Las diferencias de género en el uso de las TIC's se han **atenuado** durante el tiempo [@hargittai_differences_2006].

-   Las estudiantes mujeres tienden a presentar menores niveles de confianza pero mayores competencias que los hombres [@punter_gender_2017].
:::
...
::: notes
-   En general tienden a ganar hombres, en especializada mujeres.
...
# Split the data into two datasets: one for each type
plot_data_general <- plot_data %>% filter(type == "1. Autoeficacia General")
plot_data_specialized <- plot_data %>% filter(type == "2. Autoeficacia Especializada")

# Function to create a plot for a given type
create_plot <- function(data, title) {
  # Reorder the data by score
  data <- data %>%
    arrange(score) %>%
    mutate(CNT = factor(CNT, levels = unique(CNT)))
  
  # Create a vector of colors for the y-axis text
  unique_countries <- levels(data$CNT)
  y_axis_colors <- ifelse(unique_countries %in% c("BRA", "ARG", "CRI", "CHL", "DOM", "PAN", "URY"), "#FE3057", "#5F5758")
  
  # Create the plot
  ggplot(data, aes(y = CNT, x = score)) +
    geom_line(aes(group = CNT), size = 0.6) +
    ...
    theme(
      ...
      axis.title.x = element_blank(),
      legend.position = "top",
      axis.text.y = element_text(size = 6, color = y_axis_colors),
...
plot_general <- create_plot(plot_data_general, "1. Autoeficacia General")
plot_specialized <- create_plot(plot_data_specialized, "2. Autoeficacia Especializada")

# Combine the plots using ggarrange
combined_plot <- ggarrange(
  plot_general, plot_specialized,
  ncol = 2, nrow = 1,
...
latam_cleaveland |>
  ...
  geom_point(aes(color = sex), size = 3.5) +
...
geom_text(
  ...
  aes(
    y = CNT, 
    x = valor_mayor + 0.01,  # Position to the right of the highest point
    label = etiqueta,
    color = type  # Map to a TRUE/FALSE variable
  ),
  hjust = -0.1,
  size = 5,
  show.legend = FALSE
