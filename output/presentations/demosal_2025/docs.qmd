...
<style>
  ...
  .circular-image {
    ...
    border-radius: 50%;
    object-fit: cover;
    border: 3px solid #ccc; /* Color del borde */
  }
</style>
```
...
pisa22ict |>  
  group_by(CNT, sex) |>  
  summarise("1. Autoeficacia General" = mean(effgen, na.rm = TRUE),
...
brecha_general <- pisa22ict %>%
  ...
  mutate(brecha_general = Male - Female) %>%
...
brecha_especifica <- pisa22ict %>%
  ...
  # Calcular brecha (usa los nombres exactos de las columnas creadas por pivot_wider)
  mutate(brecha_especifica = Male - Female) %>%
...
ggplot(datos_analisis, aes(x = GDI, y = score)) +
  ...
  labs(
    y = "Ventaja de mujeres en autoeficacia Digital (Mujer - Hombre)",
